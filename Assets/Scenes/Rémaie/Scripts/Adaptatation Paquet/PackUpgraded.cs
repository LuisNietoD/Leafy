...
public class PackUpgraded : MonoBehaviour
{
    ...
    private void OnFirstButtonClick()
    {
        ...
        {
            GetCard(buttonPressCount - 1); // Passer l'indice de la carte � obtenir
        }
        else if (buttonPressCount == 6)
        {
            // Apr�s la 5e pression, d�sactive le premier bouton et active le deuxi�me bouton
            firstButton.gameObject.SetActive(false);
            buttonPressCount = 0; // R�initialise le compteur de pression de bouton
            DestroyExistingCards(); // D�truit toutes les cartes existantes
